...
function Build-VsCodeExtension([string] $packageDirectory, [string] $outputSubDirectory, [string] $packageVersionNumber, [string] $kernelVersionNumber = "") {
    ...
    # Pass --allow-package-all-secrets because the @secretlint package used by vsce is attempting to read a directory as a file.
    npx @vscode/vsce package -o "$outDir\$outputSubDirectory\dotnet-interactive-vscode-$packageVersionNumber.vsix" --allow-package-all-secrets
