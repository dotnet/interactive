...
[true, false].forEach(useLegacy => {
    describe(`metadata utility tests (${useLegacy ? 'Use Legacy metadata' : 'Use new Metadata'})`, async () => {
        beforeEach(() => {
            metadataUtilities.setUseLegacyMetadata(useLegacy);
        });
        ...
        it('interactive notebook can be identified from metadata in .ipynb', () => {
            let notebookDocument: vscodeLike.NotebookDocument = {
                ...
                metadata: metadataUtilities.useLegacyMetadata() ? {
                    custom: {
                        metadata: {
                            kernelspec: {
                                name: '.net-csharp'
        ...
        it('new ipynb metadata can be created from existing data', () => {
            ...
            const existingMetadata: { [key: string]: any } = metadataUtilities.useLegacyMetadata() ? {
                custom: {
                    metadata: {
                ...
                expect(newRawMetadata).to.deep.equal({
                    metadata:
                    {
                        ...
                        polyglot_notebook:
                        {
                            ...
                            {
                                defaultKernelName: 'csharp',
                                items: [{ aliases: ['cs'], languageName: 'csharp', name: 'csharp' }]
                            }
                        },
                        someKey: 'some value'
                    },
