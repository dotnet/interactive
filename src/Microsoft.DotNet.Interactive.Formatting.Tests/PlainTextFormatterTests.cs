...
public class PlainTextFormatterTests : FormatterTestBase
{
    public class Objects : FormatterTestBase
    {
        ...
        public void boom()
        {
            ...
            var content = File.ReadAllText("./kindergartens.geojson");

            var data = JsonSerializer.Deserialize<NetTopologySuite.Features.FeatureCollection>(content, opt);
            Formatter.ListExpansionLimit = 1;

            data.Invoking(d =>
            {
                var toFormat = (d[1].Geometry) as NetTopologySuite.Geometries.GeometryCollection;
                toFormat?.ToDisplayString(PlainTextFormatter.MimeType);
            }).Should().NotThrow();
        ...
        [Fact]
        public void Recursive_formatter_calls_do_not_cause_exceptions_2()
        {
            var list = new LinkedListNode();
            var root = list;
                
            for (var i = 0; i < 1783; i++)
            {
                root.Next = new LinkedListNode();
                root = root.Next;   
            }

            var formatter = PlainTextFormatter.GetPreferredFormatterFor(list.GetType());

            list.Invoking(w => w.ToDisplayString(formatter)).Should().NotThrow();
        }

        [Fact]
        public void Recursive_formatter_calls_do_not_cause_exceptions_3()
        {
            object list = new FeatureCollection
            {
                new Feature
                {
            ...
            list.Invoking(w => w.ToDisplayString(PlainTextFormatter.MimeType)).Should().NotThrow();
        }
