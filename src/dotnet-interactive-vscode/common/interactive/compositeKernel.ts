...
import { KernelCommandEnvelope, KernelCommand } from "../interfaces/contracts";
...
export class CompositeKernel extends Kernel {
    ...
    handleCommand(commandEnvelope: KernelCommandEnvelope): Promise<void> {
        if (commandEnvelope.command.targetKernelName === this.name) {
            return super.handleCommand(commandEnvelope);
        }

        let kernel = this.getTargetKernel(commandEnvelope.command);
