...
public class PlainTextFormatter<T> : TypeFormatter<T>
{
    ...
    {
        using var _ = Disposable.Create(() => Console.WriteLine($"Leaving PlainTextFormatter<{typeof(T).Name}>.{nameof(CreateForAnyObject)} for type {typeof(T).Name}"));
        Console.WriteLine($"Entering PlainTextFormatter<{typeof(T).Name}>.{nameof(CreateForAnyObject)} for type {typeof(T).Name}");
    ...
    {
        using var _ = Disposable.Create(() => Console.WriteLine($"Leaving PlainTextFormatter<{typeof(T).Name}>.{nameof(CreateForAnyEnumerable)} for type {typeof(T).Name}"));
        Console.WriteLine($"Entering PlainTextFormatter<{typeof(T).Name}>.{nameof(CreateForAnyEnumerable)} for type {typeof(T).Name}");
                ...
                .FirstOrDefault() is { } t)
        {
            ...
            var joinDelegate = genericMethod.CreateDelegate(delegateType);

            return new((value, context) =>
            {
                using var _ = Disposable.Create(() => Console.WriteLine($"Leaving PlainTextFormatter<{typeof(T).Name}>.DynamicInvoke for type {value.GetType().Name}"));
                Console.WriteLine($"Entering PlainTextFormatter<{typeof(T).Name}>.DynamicInvoke for type {value.GetType().Name}");
                joinDelegate.DynamicInvoke(value, context.Writer, context, Formatter<T>.ListExpansionLimit);
                return true;
            });
        }
