...
namespace Microsoft.DotNet.Interactive.Tests.LanguageServices
{
    public class CompletionTests : LanguageKernelTestBase
    {
        ...
        [Theory]
        [InlineData("[|#!d|]", "#!directiveOnParent", null)]
        [InlineData("[|#!dir|]\n", "#!directiveOnParent", null)]
        ...
        public async Task Completions_are_available_for_magic_commands_added_at_runtime_to_child_and_parent_kernels(
            string markupCode,
            string expected,
            Language? childKernel)
        {
            MarkupTestFile.GetSpan(markupCode, out var code, out var span);

            var sourceText = SourceText.From(code);

            var kernel = CreateKernel(childKernel ?? Language.CSharp);
            ...
            if (childKernel != null)
            {
                var kernelToExtend = kernel.FindKernel(childKernel.Value.LanguageName()) as KernelBase;
