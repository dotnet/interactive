...
public class CommandRoutingSlipTests
{
    ...
    public void can_be_stamped_with_kernel_uri_on_arrival()
    {
        ...
        routingSlip.StampAsArrived(new Uri("kernel://pid-1"));
        routingSlip.StartsWith(new Uri("kernel://pid-1")).Should().BeTrue();
    }
    ...
    public void the_uri_array_contains_only_stamped_kernel_uri()
    {
        var routingSlip = new CommandRoutingSlip();
        routingSlip.StampAsArrived(new Uri("kernel://pid-1"));
        routingSlip.Stamp(new Uri("kernel://pid-1"));
        routingSlip.StampAsArrived(new Uri("kernel://pid-2"));
        routingSlip.Stamp(new Uri("kernel://pid-2"));
        routingSlip.StampAsArrived(new Uri("kernel://pid-3"));

        routingSlip.ToUriArray().Should().ContainInOrder(
            new Uri("kernel://pid-1"),
            new Uri("kernel://pid-2"));
    }
    ...
    public void cannot_can_be_stamped_with_kernel_uri_on_arrival_twice()
    {
        ...
        var markAgain = () => routingSlip.StampAsArrived(new Uri("kernel://pid-1"));

        markAgain.Should().ThrowExactly<InvalidOperationException>()
            .WithMessage("The uri kernel://pid-1/ is already in the routing slip");
    }
    ...
    public void cannot_can_be_stamped_with_kernel_uri_if_the_uri_is_not_stamped_on_arrival()
    {
        ...
        var markAgain = () => routingSlip.Stamp(new Uri("kernel://pid-1"));

        markAgain.Should().ThrowExactly<InvalidOperationException>()
            .WithMessage("The uri kernel://pid-1/ is not in the routing slip or has already been completed");
    }
    ...
    public void can_append_a_routingSlip_to_another()
    {
        ...
        original.FullStamp(new Uri("kernel://pid-1"));
        original.FullStamp(new Uri("kernel://pid-2"));


        var toBeAppended = new CommandRoutingSlip();
        toBeAppended.FullStamp(new Uri("kernel://pid-3"));
        toBeAppended.FullStamp(new Uri("kernel://pid-4"));
        ...
        original.ToUriArray().Should().ContainInOrder(
            new Uri("kernel://pid-1"),
            new Uri("kernel://pid-2"),
            new Uri("kernel://pid-3"),
            new Uri("kernel://pid-4"));
    }
    ...
    public void can_append_a_routingSlip_to_another_skipping_entries_if_the_other_contains_it()
    {
        ...
        var toBeAppended = new CommandRoutingSlip();
        toBeAppended.FullStamp(new Uri("kernel://pid-1"));
        toBeAppended.FullStamp(new Uri("kernel://pid-2"));
        toBeAppended.FullStamp(new Uri("kernel://pid-3"));
        toBeAppended.FullStamp(new Uri("kernel://pid-4"));
    ...
    public void starts_with_urls(Uri[] other, bool startsWith)
    {
        ...
        original.FullStamp(new Uri("kernel://pid-1"));
        original.FullStamp(new Uri("kernel://pid-2"));
        original.FullStamp(new Uri("kernel://pid-3"));
    ...
    public static IEnumerable<object[]> EventRoutingSlipsToTest()
    {
        ...
        yield return new object[] { new[] { new Uri("kernel://pid-1") }, true };
        yield return new object[] { new[] { new Uri("kernel://pid-1"), new Uri("kernel://pid-2") }, true };
        yield return new object[] { new[] { new Uri("kernel://pid-1"), new Uri("kernel://pid-2"), new Uri("kernel://pid-3") }, true };
        yield return new object[] { new[] { new Uri("kernel://pid-1"), new Uri("kernel://pid-2"), new Uri("kernel://pid-4") }, false };
    }
    ...
    public void can_append_a_routingSlip_to_another_contains_only_fully_Stamped_kernel_uris()
    {
        ...
        toBeAppended.FullStamp(new Uri("kernel://pid-2"));
        toBeAppended.FullStamp(new Uri("kernel://pid-3"));
        toBeAppended.StampAsArrived(new Uri("kernel://pid-4"));
    ...
    public void fails_to_append_a_routingSlip_to_another_if_they_do_not_start_with_same_uris()
    {
        ...
        appendAction.Should().ThrowExactly<InvalidOperationException>().WithMessage("The uri kernel://pid-1/ is already in the routing slip");
    }
